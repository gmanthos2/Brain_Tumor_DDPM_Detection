...
\paragraph{Inference Complexity and Efficiency.} The computational cost of inference is dominated by the iterative DDIM reverse process. Each DDIM step requires one U-Net forward pass, and Classifier-Free Guidance doubles this to two forward passes per step (one conditional, one unconditional). For our default configuration ($S = 50$ DDIM steps with CFG), anomaly detection thus requires $2S = 100$ U-Net evaluations plus one VAE encode and one VAE decode. Total inference complexity scales as $\mathcal{O}(S \cdot \sum_{\ell} C_\ell^2 \cdot H_\ell W_\ell)$, where $C_\ell$ and $H_\ell \times W_\ell$ are the channel count and spatial resolution at each U-Net level~$\ell$. The key efficiency gain of latent diffusion is that the U-Net operates on $32 \times 32$ latent maps rather than $256 \times 256$ pixel-space images, reducing the spatial area by a factor of $64\times$ and yielding an estimated ${\sim}50\times$ reduction in per-pass FLOPs compared to an equivalent pixel-space diffusion model. Combined with DDIM's $20\times$ step reduction relative to full $T = 1000$ DDPM sampling, the overall inference cost is approximately three orders of magnitude lower than a na\"{i}ve pixel-space DDPM baseline. In practice, on our NVIDIA RTX 4060, a single anomaly detection pass completes in approximately 300--350~ms per image (${\sim}3$~images/s), making the pipeline feasible for near-interactive clinical review on consumer-grade hardware. The total model footprint comprises approximately 35M parameters (U-Net) and 17M parameters (VAE), totaling ${\sim}52$M trainable parameters.
...
Furthermore, the deployment of generative AI models in medical imaging raises important ethical concerns that must be addressed alongside technical contributions.
